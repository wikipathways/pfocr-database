...
6. run `bundle exec jekyll build` <-- run this to generate _site files without localhost URLs. Might need to run it as `JEKYLL_ENV=production bundle exec jekyll build` to make sure the sitemap doesn't use localhost URLs.
