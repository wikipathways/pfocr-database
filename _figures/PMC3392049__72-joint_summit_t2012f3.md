---
figid: PMC3392049__72-joint_summit_t2012f3
figlink: /pmc/articles/PMC3392049/figure/f3-72-joint_summit_t2012/
number: F3
caption: An example pathway. The shown ingenuity pathway (Prostate Cancer Signaling
  Pathway) overlaps significantly with the mutation-prone subnetwork corresponding
  to the expression module annotated by GO:0008285. The overlapping genes are shown
  as shaded nodes.
pmcid: PMC3392049
papertitle: Integrating Genome and Functional Genomics Data to Reveal Perturbed Signaling
  Pathways in Ovarian Cancers.
reftext: Songjian Lu, et al. AMIA Jt Summits Transl Sci Proc. 2012;2012:72-78.
pmc_ranked_result_index: '3566'
pathway_score: 0.7685007
filename: 72-joint_summit_t2012f3.jpg
figtitle: An example pathway
year: '2012'
organisms:
- Saccharomyces cerevisiae
- Homo sapiens
ndex: c82dec05-df28-11ea-99da-0ac135e8bacf
annotations: []
seo: CreativeWork
schema-jsonld:
  '@context': https://schema.org/
  '@id': https://pfocr.wikipathways.org/figures/PMC3392049__72-joint_summit_t2012f3.html
  '@type': Dataset
  description: An example pathway. The shown ingenuity pathway (Prostate Cancer Signaling
    Pathway) overlaps significantly with the mutation-prone subnetwork corresponding
    to the expression module annotated by GO:0008285. The overlapping genes are shown
    as shaded nodes.
  license: CC0
  name: CreativeWork
  creator:
    '@type': Organization
    name: WikiPathways
  keywords:
  - CIB1
  - MAP2K1
  - MLIP
  - MAPK3
  - LEF1
  - MDM2
  - CREB5
  - GSTP1
  - PIK3R6
  - OPN1LW
  - E2F1
  - PIK3CA
  - MAPK1
  - BCL2
  - PIK3R3
  - RB1
  - CREB3L3
  - CREB3L1
  - CREM
  - PIK3CB
  - PTEN
  - CREB3L2
  - AR
  - CREB3L4
  - ATF2
  - PIK3R5
  - CREB1
  - MAP2K2
  - PIK3R4
  - PIK3CD
  - PIK3CG
  - ATF1
  - CREB3
  - FAM3B
  - Testosterone
  - Testosterone Testosterone
  - Cancer
genes:
- word: KIP
  symbol: KIP
  source: hgnc_alias_symbol
  hgnc_symbol: CIB1
  entrez: '10519'
- word: MEK
  symbol: MEK
  source: bioentities_symbol
  hgnc_symbol: MAP2K1
  entrez: '5604'
- word: CIP
  symbol: CIP
  source: hgnc_alias_symbol
  hgnc_symbol: MLIP
  entrez: '90523'
- word: ERK
  symbol: ERK
  source: bioentities_symbol
  hgnc_symbol: MAPK3
  entrez: '5595'
- word: LEF1
  symbol: LEF1
  source: hgnc_symbol
  hgnc_symbol: LEF1
  entrez: '51176'
- word: MDM2
  symbol: MDM2
  source: hgnc_symbol
  hgnc_symbol: MDM2
  entrez: '4193'
- word: CREB
  symbol: CREB
  source: bioentities_symbol
  hgnc_symbol: CREB5
  entrez: '9586'
- word: GSTP1
  symbol: GSTP1
  source: hgnc_symbol
  hgnc_symbol: GSTP1
  entrez: '2950'
- word: PI3K
  symbol: PI3K
  source: bioentities_symbol
  hgnc_symbol: PIK3R6
  entrez: '146850'
- word: CBP
  symbol: CBP
  source: hgnc_prev_symbol
  hgnc_symbol: OPN1LW
  entrez: '5956'
- word: E2F1
  symbol: E2F1
  source: hgnc_symbol
  hgnc_symbol: E2F1
  entrez: '1869'
- word: PI3K
  symbol: PI3K
  source: bioentities_symbol
  hgnc_symbol: PIK3CA
  entrez: '5290'
- word: ERK
  symbol: ERK
  source: bioentities_symbol
  hgnc_symbol: MAPK1
  entrez: '5594'
- word: BCL2
  symbol: BCL2
  source: hgnc_symbol
  hgnc_symbol: BCL2
  entrez: '596'
- word: PI3K
  symbol: PI3K
  source: bioentities_symbol
  hgnc_symbol: PIK3R3
  entrez: '8503'
- word: RB1
  symbol: RB1
  source: hgnc_symbol
  hgnc_symbol: RB1
  entrez: '5925'
- word: CREB
  symbol: CREB
  source: bioentities_symbol
  hgnc_symbol: CREB3L3
  entrez: '84699'
- word: CREB
  symbol: CREB
  source: bioentities_symbol
  hgnc_symbol: CREB3L1
  entrez: '90993'
- word: CREB
  symbol: CREB
  source: bioentities_symbol
  hgnc_symbol: CREM
  entrez: '1390'
- word: PI3K
  symbol: PI3K
  source: bioentities_symbol
  hgnc_symbol: PIK3CB
  entrez: '5291'
- word: PTEN
  symbol: PTEN
  source: hgnc_symbol
  hgnc_symbol: PTEN
  entrez: '5728'
- word: CREB
  symbol: CREB
  source: bioentities_symbol
  hgnc_symbol: CREB3L2
  entrez: '64764'
- word: AR
  symbol: AR
  source: hgnc_symbol
  hgnc_symbol: AR
  entrez: '367'
- word: CREB
  symbol: CREB
  source: bioentities_symbol
  hgnc_symbol: CREB3L4
  entrez: '148327'
- word: CREB
  symbol: CREB
  source: bioentities_symbol
  hgnc_symbol: ATF2
  entrez: '1386'
- word: PI3K
  symbol: PI3K
  source: bioentities_symbol
  hgnc_symbol: PIK3R5
  entrez: '23533'
- word: CREB
  symbol: CREB
  source: bioentities_symbol
  hgnc_symbol: CREB1
  entrez: '1385'
- word: MEK
  symbol: MEK
  source: bioentities_symbol
  hgnc_symbol: MAP2K2
  entrez: '5605'
- word: PI3K
  symbol: PI3K
  source: bioentities_symbol
  hgnc_symbol: PIK3R4
  entrez: '30849'
- word: PI3K
  symbol: PI3K
  source: bioentities_symbol
  hgnc_symbol: PIK3CD
  entrez: '5293'
- word: PI3K
  symbol: PI3K
  source: bioentities_symbol
  hgnc_symbol: PIK3CG
  entrez: '5294'
- word: CREB
  symbol: CREB
  source: bioentities_symbol
  hgnc_symbol: ATF1
  entrez: '466'
- word: CREB
  symbol: CREB
  source: bioentities_symbol
  hgnc_symbol: CREB3
  entrez: '10488'
- word: '221'
  symbol: 2-21
  source: hgnc_alias_symbol
  hgnc_symbol: FAM3B
  entrez: '54097'
chemicals:
- word: Testosterone
  source: MESH
  identifier: D013739
- word: Testosterone Testosterone
  source: MESH
  identifier: D013739
diseases:
- word: Cancer
  source: ''
  identifier: ''
figid_alias: PMC3392049__F3
redirect_from: /figures/PMC3392049__F3
figtype: Figure
---
