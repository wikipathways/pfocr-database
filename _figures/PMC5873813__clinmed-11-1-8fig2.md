---
figid: PMC5873813__clinmed-11-1-8fig2
figlink: /pmc/articles/PMC5873813/figure/figure2/
number: Fig 2
caption: Broad overview of the genetic architecture of inflammatory bowel disease
  giving examples of confirmed and candidate genes implicated in each pathway. T®
  = regulatory T cell.
pmcid: PMC5873813
papertitle: 'Genes in inflammatory bowel disease: lessons from complex diseases.'
reftext: Paul Henderson, et al. Clin Med (Lond). 2011 Feb;11(1):8-10.
pmc_ranked_result_index: '4909'
pathway_score: 0.6505652
filename: clinmed-11-1-8fig2.jpg
figtitle: Broad overview of the genetic architecture of inflammatory bowel disease
  giving examples of confirmed and candidate genes implicated in each pathway
year: '2011'
organisms:
- Homo sapiens
ndex: ''
annotations: []
seo: CreativeWork
schema-jsonld:
  '@context': https://schema.org/
  '@id': https://pfocr.wikipathways.org/figures/PMC5873813__clinmed-11-1-8fig2.html
  '@type': Dataset
  description: Broad overview of the genetic architecture of inflammatory bowel disease
    giving examples of confirmed and candidate genes implicated in each pathway. T®
    = regulatory T cell.
  license: CC0
  name: CreativeWork
  creator:
    '@type': Organization
    name: WikiPathways
  keywords:
  - JAK2
  - SMAD3
  - FUT2
  - ICOSLG
  - TLR4
  - TNFRSF6B
  - TYK2
  - IRGM
  - ORMDL3
  - CCR6
  - LRRK2
  - XBP1
  - TNFSF15
  - IL12B
  - STAT3
  - TAGAP
  - IL27
  - CAPN10
  - IL22
  - IL23R
  - ATG16L1
  - NOD2
  - ERAP2
  - MST1
  - MTMR3
  - Crohn's disease
genes:
- word: JAK2
  symbol: JAK2
  source: hgnc_symbol
  hgnc_symbol: JAK2
  entrez: '3717'
- word: SMAD3
  symbol: SMAD3
  source: hgnc_symbol
  hgnc_symbol: SMAD3
  entrez: '4088'
- word: FUT2
  symbol: FUT2
  source: hgnc_symbol
  hgnc_symbol: FUT2
  entrez: '2524'
- word: ICOSLG
  symbol: ICOSLG
  source: hgnc_symbol
  hgnc_symbol: ICOSLG
  entrez: '23308'
- word: TLR4
  symbol: TLR4
  source: hgnc_symbol
  hgnc_symbol: TLR4
  entrez: '7099'
- word: TNFRSF6B
  symbol: TNFRSF6B
  source: hgnc_symbol
  hgnc_symbol: TNFRSF6B
  entrez: '8771'
- word: TYK2
  symbol: TYK2
  source: hgnc_symbol
  hgnc_symbol: TYK2
  entrez: '7297'
- word: IRGM
  symbol: IRGM
  source: hgnc_symbol
  hgnc_symbol: IRGM
  entrez: '345611'
- word: ORMDL3
  symbol: ORMDL3
  source: hgnc_symbol
  hgnc_symbol: ORMDL3
  entrez: '94103'
- word: CCR6
  symbol: CCR6
  source: hgnc_symbol
  hgnc_symbol: CCR6
  entrez: '1235'
- word: LRRK2
  symbol: LRRK2
  source: hgnc_symbol
  hgnc_symbol: LRRK2
  entrez: '120892'
- word: XBP1
  symbol: XBP1
  source: hgnc_symbol
  hgnc_symbol: XBP1
  entrez: '7494'
- word: TL1A
  symbol: TL1A
  source: hgnc_alias_symbol
  hgnc_symbol: TNFSF15
  entrez: '9966'
- word: IL-12B
  symbol: IL-12B
  source: hgnc_alias_symbol
  hgnc_symbol: IL12B
  entrez: '3593'
- word: STAT3
  symbol: STAT3
  source: hgnc_symbol
  hgnc_symbol: STAT3
  entrez: '6774'
- word: TAGAP
  symbol: TAGAP
  source: hgnc_symbol
  hgnc_symbol: TAGAP
  entrez: '117289'
- word: IL27
  symbol: IL27
  source: hgnc_symbol
  hgnc_symbol: IL27
  entrez: '246778'
- word: CAPN10
  symbol: CAPN10
  source: hgnc_symbol
  hgnc_symbol: CAPN10
  entrez: '11132'
- word: IL-22
  symbol: IL22
  source: hgnc_symbol
  hgnc_symbol: IL22
  entrez: '50616'
- word: IL23R
  symbol: IL23R
  source: hgnc_symbol
  hgnc_symbol: IL23R
  entrez: '149233'
- word: ATG16L1
  symbol: ATG16L1
  source: hgnc_symbol
  hgnc_symbol: ATG16L1
  entrez: '55054'
- word: NOD2
  symbol: NOD2
  source: hgnc_symbol
  hgnc_symbol: NOD2
  entrez: '64127'
- word: ERAP2
  symbol: ERAP2
  source: hgnc_symbol
  hgnc_symbol: ERAP2
  entrez: '64167'
- word: MST1
  symbol: MST1
  source: hgnc_symbol
  hgnc_symbol: MST1
  entrez: '4485'
- word: MTMR3
  symbol: MTMR3
  source: hgnc_symbol
  hgnc_symbol: MTMR3
  entrez: '8897'
chemicals: []
diseases:
- word: Crohn's disease
  source: ''
  identifier: ''
---
