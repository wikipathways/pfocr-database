---
figid: PMC8104964__elife-65856-fig1-figsupp4
figtitle: TLR4 signaling pathways
organisms:
- NA
organisms_ner:
- Homo sapiens
pmcid: PMC8104964
filename: elife-65856-fig1-figsupp4.jpg
figlink: /pmc/articles/PMC8104964/figure/fig1s4/
number: SF1_4
caption: LPS binding to TLR4 at the cell surface initially activates the MYD88-dependent
  signaling pathway, which results in activation of NF-κB and production of pro-inflammatory
  cytokines such as TNF-α. Endocytosis of the LPS-TLR4 complex leads to activation
  of the TRIF-dependent signaling pathway, which both further activates NF-κB and
  initiates an interferon response. Labels of genes mentioned in the main text are
  bolded. For genes with mismatched protein and gene names, protein names are indicated
  with gene names in parentheses.
papertitle: CRISPR-based functional genomics in human dendritic cells.
reftext: Marco Jost, et al. eLife. 2021;10:e65856.
year: '2021'
doi: 10.7554/eLife.65856
journal_title: eLife
journal_nlm_ta: eLife
publisher_name: eLife Sciences Publications, Ltd
keywords: CRISPR | dendritic cells | bacteroides thetaiotaomicron | functional genomics
  | inter-individual variation | Human | Other
automl_pathway: 0.9339235
figid_alias: PMC8104964__SF1_4
figtype: Supplemental figure
redirect_from: /figures/PMC8104964__SF1_4
ndex: ''
seo: CreativeWork
schema-jsonld:
  '@context': https://schema.org/
  '@id': https://pfocr.wikipathways.org/figures/PMC8104964__elife-65856-fig1-figsupp4.html
  '@type': Dataset
  description: LPS binding to TLR4 at the cell surface initially activates the MYD88-dependent
    signaling pathway, which results in activation of NF-κB and production of pro-inflammatory
    cytokines such as TNF-α. Endocytosis of the LPS-TLR4 complex leads to activation
    of the TRIF-dependent signaling pathway, which both further activates NF-κB and
    initiates an interferon response. Labels of genes mentioned in the main text are
    bolded. For genes with mismatched protein and gene names, protein names are indicated
    with gene names in parentheses.
  license: CC0
  name: CreativeWork
  creator:
    '@type': Organization
    name: WikiPathways
  keywords:
  - CD14
  - NDUFA2
  - IRF6
  - LBP
  - EHHADH
  - RPSA
  - LY96
  - TLR4
  - FOS
  - FOSB
  - JUN
  - JUNB
  - JUND
  - SEMA6A
  - TBK1
  - TIRAP
  - MYD88
  - TRAF6
  - NFKB1
  - RELA
  - RELB
  - REL
  - NFKB2
  - TRAF3
  - TNF
  - TICAM2
  - TMED7-TICAM2
  - TRAM1
  - TICAM1
  - TRIM69
  - IRF3
  - IRF7
  - IFNB1
  - IFNK
  - IFNW1
  - IFNA1
  - IFNA2
  - IFNA4
  - IFNA5
  - IFNA6
  - IFNA7
  - IFNA8
  - IFNA10
  - IFNA13
  - IFNA14
  - IFNA16
  - IFNA17
  - IFNA21
  - CXCL10
---
