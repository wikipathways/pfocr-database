---
figid: PMC2652055__jexbotern336f01_3c
figtitle: Simplified overview of flavonol and anthocyanin biosynthesis within the
  phenylpropanoid pathway and its regulation in grape by characterized MYB genes (flavan-3-ols
  are not shown in this pathway)
organisms:
- NA
pmcid: PMC2652055
filename: jexbotern336f01_3c.jpg
figlink: /pmc/articles/PMC2652055/figure/fig1/
number: F1
caption: 'Simplified overview of flavonol and anthocyanin biosynthesis within the
  phenylpropanoid pathway and its regulation in grape by characterized MYB genes (flavan-3-ols
  are not shown in this pathway). The repressor MYB4 is shown in red, while all MYB
  activators are shown in green. Abbreviations: CHS, chalcone synthase; CHI, chalcone
  isomerase; F3H/F3′H/F3′5′H, flavonoid hydroxylases; DFR, dihydroflavonol-4-reductase;
  ANS/LDOX, anthocyanidin synthase/leucoanthocyanidin dioxygenase; UFGT, UDP glucose:flavonoid-3-O-glucosyltransferase;
  FLS, flavonol synthase; and OMT, O-methyltransferase. (This figure is available
  in colour at JXB online.)'
papertitle: Post-veraison sunlight exposure induces MYB-mediated transcriptional regulation
  of anthocyanin and flavonol synthesis in berry skins of Vitis vinifera.
reftext: José Tomás Matus, et al. J Exp Bot. 2009 Mar;60(3):853-867.
year: '2009'
doi: .na.character
journal_title: .na.character
journal_nlm_ta: .na.character
publisher_name: .na.character
keywords: .na.character
automl_pathway: 0.9447977
figid_alias: PMC2652055__F1
figtype: Figure
organisms_ner:
- Arabidopsis thaliana
redirect_from: /figures/PMC2652055__F1
ndex: ''
seo: CreativeWork
schema-jsonld:
  '@context': https://schema.org/
  '@id': https://pfocr.wikipathways.org/figures/PMC2652055__jexbotern336f01_3c.html
  '@type': Dataset
  description: 'Simplified overview of flavonol and anthocyanin biosynthesis within
    the phenylpropanoid pathway and its regulation in grape by characterized MYB genes
    (flavan-3-ols are not shown in this pathway). The repressor MYB4 is shown in red,
    while all MYB activators are shown in green. Abbreviations: CHS, chalcone synthase;
    CHI, chalcone isomerase; F3H/F3′H/F3′5′H, flavonoid hydroxylases; DFR, dihydroflavonol-4-reductase;
    ANS/LDOX, anthocyanidin synthase/leucoanthocyanidin dioxygenase; UFGT, UDP glucose:flavonoid-3-O-glucosyltransferase;
    FLS, flavonol synthase; and OMT, O-methyltransferase. (This figure is available
    in colour at JXB online.)'
  license: CC0
  name: CreativeWork
  creator:
    '@type': Organization
    name: WikiPathways
  keywords:
  - TT4
  - CHI
  - TT5
  - FH
  - MYB12
  - FLS1
  - FLS3
  - DFR
  - LDOX
  - MYB4
  - Cyanidin
  - Delphinidin
  - Myricetin
  - Malvidin
  - Peonidin
  - Petunidin
  - glucoside
  - Quercetin
  - Kaempferol
---
